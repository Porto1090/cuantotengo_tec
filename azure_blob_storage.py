...
def save_image_to_blob(image_rgb, session_id, prefix="images"):
    img_bytes = cv2.imencode(".jpg", cv2.cvtColor(image_rgb, cv2.COLOR_RGB2BGR))[1].tobytes()

    blob_name = f"{prefix}/{session_id}.jpg"
...
def save_log_to_blob(log_dict, session_id, prefix="logs"):
    blob_name = f"{prefix}/{session_id}.json"
    blob = blob_client.get_blob_client(container=container, blob=blob_name)
